      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed bottom-6 right-6 w-96 h-[500px] z-50 glass rounded-2xl border border-white/20 shadow-2xl overflow-hidden"
          >
            {/* Chat Header */}
            ...
            {/* Messages Area */}
            <div className="flex-1 p-4 h-80 overflow-y-auto space-y-4 bg-dark-900/50">
                ...
                <div className="flex flex-wrap gap-2">
                  {quickReplies.map((reply, index) => (
                    <motion.button
                      ...
                      className="text-xs px-3 py-2 glass rounded-full border border-accent-blue/30 hover:border-accent-blue/50 transition-all duration-300 text-accent-blue"
                    >
                      {reply}
                    </motion.button>
                  ))}
            ...
            <div className="p-4 border-t border-white/10 bg-dark-800/50">
              <div className="flex space-x-2">
                <input
                  ...
                  className="flex-1 px-4 py-2 bg-dark-700 border border-white/20 rounded-xl focus:border-accent-blue focus:outline-none text-sm"
                  disabled={isTyping}
                />
                <motion.button
                  whileHover={{ scale: 1.05 }}
